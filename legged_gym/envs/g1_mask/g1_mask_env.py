...
class G1Robot_Masked(LeggedRobot):
    ...
    def _init_buffers(self):
        ...
        self.dof_pos = self.dof_state.view(self.num_envs, G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num, 2)[..., 0]
        ...
        self.dof_vel = self.dof_state.view(self.num_envs, G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num, 2)[..., 1]
        ...
        self.torques = torch.zeros(self.num_envs, G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num, dtype=torch.float, device=self.device, requires_grad=False)
        self.p_gains = torch.zeros(G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num, dtype=torch.float, device=self.device, requires_grad=False)
        self.d_gains = torch.zeros(G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        print("======g1_mask_env.py:_init_buffers======\nself.height_points.shape:",self.height_points.shape)
        ##

        # joint positions offsets and PD gains
        # self.default_dof_pos = torch.zeros(self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        self.default_dof_pos = torch.zeros(G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        # for i in range(self.num_dofs):
        for i in range(self.num_all_dofs):
            name = self.dof_names[i]
            ...
            # print("Now name is ",name," and angle is ",angle)
            self.default_dof_pos[i] = angle
            found = False
            for dof_name in self.cfg.control.stiffness.keys():
                # print("Now dof_name is ",dof_name)
                if dof_name in name:
                    # print(f"Found dof_name: {dof_name} in name: {name}")
                    self.p_gains[i] = self.cfg.control.stiffness[dof_name]
                    self.d_gains[i] = self.cfg.control.damping[dof_name]
                    found = True
            if not found:
                print(f"Warning: PD gain of joint {name} were not defined, setting them to zero")
                self.p_gains[i] = 0.
                self.d_gains[i] = 0.
    ...
    def compute_observations(self):
        ...
        #                             cos_phase
        #                             ),dim=-1)
        delta_dof_pos = ((self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos)[...,:12]
        delta_dof_vel = (self.dof_vel * self.obs_scales.dof_vel)[...,:12]
        # print("delta_dof_pos.shape=",delta_dof_pos.shape)
        # print("delta_dof_vel.shape=",delta_dof_vel.shape)
        self.obs_buf = torch.cat((  self.base_ang_vel  * self.obs_scales.ang_vel,
                                    self.projected_gravity,
                                    self.commands[:, :3] * self.commands_scale,
                                    delta_dof_pos,
    ...
    def step(self, actions):
        ...
        de=Debugger()
        # de.dprint("g1_mask_env.py", "step", "actions.shape:", self.actions.shape, "\nactions:", self.actions)
        self.extend_actions = torch.cat([self.actions, torch.zeros(self.actions.size(0), G1RoughCfg_Masked.masked_dof.num, device=self.actions.device, dtype=self.actions.dtype)], dim=1)
        for i in range(G1RoughCfg_Masked.masked_dof.num):
            name = G1RoughCfg_Masked.masked_dof.name[i]
            action_idx=G1RoughCfg_Masked.masked_dof.id[i]
            if name == 'left_shoulder_pitch_joint':
                freq = 0.5
                elapsed_time = self.gym.get_elapsed_time(self.sim)
                base_phase = np.pi/2 + 2 * np.pi * freq * elapsed_time
                self.extend_actions[:,action_idx] = np.pi/4 *np.sin(base_phase-np.pi/2)
            elif name == 'right_shoulder_pitch_joint':
                freq = 0.5
                elapsed_time = self.gym.get_elapsed_time(self.sim)
                base_phase = np.pi/2 + 2 * np.pi * freq * elapsed_time
                self.extend_actions[:,action_idx] = np.pi/4 *np.sin(base_phase+np.pi/2)
        # self.extend_actions = torch.cat([torch.zeros(self.actions.size(0), G1RoughCfg_Masked.masked_dof.num, device=self.device, dtype=self.actions.dtype), self.actions], dim=1)
        # de.dprint("g1_mask_env.py", "step", "extend_actions.shape:", self.extend_actions.shape,"\nextend_actions:", self.extend_actions)
        # step physics and render each frame
        self.render()
        for _ in range(self.cfg.control.decimation):
            # self.torques = self._compute_torques(self.actions).view(self.torques.shape)
            self.torques = self._compute_torques(self.extend_actions).view(self.torques.shape)
            # de.dprint("g1_mask_env.py", "step", "self.torques.shape:", self.torques.shape, "\nself.torques:", self.torques)
            # 传入masked_cfg中的default_torques:
            # masked_values_tensor= torch.tensor(G1RoughCfg_Masked.masked_dof.default_torques, device=self.torques.device, dtype=self.torques.dtype)
            # masked_torques = masked_values_tensor.expand(self.torques.shape[0], -1)
            # self.full_torques = torch.cat([self.torques, masked_torques], dim=1)
            ...
            # self.full_torques = torch.cat([torch.zeros(self.torques.size(0), G1RoughCfg_Masked.unmask_dof.num, device=self.torques.device, dtype=self.torques.dtype), self.torques], dim=1)
            # de.dprint("g1_mask_env.py", "step", "self.full_torques.shape:", self.full_torques.shape, "\nself.full_torques:", self.full_torques)
        ...
        if self.privileged_obs_buf is not None:
            ...
            # print("self.privileged_obs_buf.shape:", self.privileged_obs_buf.shape)
    ...
    def _process_dof_props(self, props, env_id):
            ...
            self.num_dofs = G1RoughCfg_Masked.unmask_dof.num
            self.num_dof=self.num_dofs
            self.num_all_dofs = G1RoughCfg_Masked.unmask_dof.num + G1RoughCfg_Masked.masked_dof.num
            ...
            if env_id==0:
                ...
                self.dof_pos_limits = torch.zeros(self.num_all_dofs, 2, dtype=torch.float, device=self.device, requires_grad=False)
                self.dof_vel_limits = torch.zeros(self.num_all_dofs, dtype=torch.float, device=self.device, requires_grad=False)
                self.torque_limits = torch.zeros(self.num_all_dofs, dtype=torch.float, device=self.device, requires_grad=False)
                ...
                for i in range(min(len(props),self.num_all_dofs)):
                    self.dof_pos_limits[i, 0] = props["lower"][i].item()
                    self.dof_pos_limits[i, 1] = props["upper"][i].item()
                    self.dof_vel_limits[i] = props["velocity"][i].item()
                    self.torque_limits[i] = props["effort"][i].item()
    ...
    def _compute_torques(self, actions):
            ...
            if control_type=="P":
                ...
                # print("torque.shape:",torques.shape)
            elif control_type=="V":
    ...
    def _reset_dofs(self, env_ids):
        ...
        self.dof_pos[env_ids] = self.default_dof_pos * torch_rand_float(0.5, 1.5, (len(env_ids), G1RoughCfg_Masked.unmask_dof.num+G1RoughCfg_Masked.masked_dof.num), device=self.device)
    ...
    def get_rand_action(self):
        ...
        # 计算当前相位: 初始相位 + 2π * 频率 * 经过时间
        current_phase = self.start_phase + 2 * np.pi * self.frequency * elapsed_time
        
        # 计算正弦波动作值
        action = self.amplitude * np.sin(current_phase)
